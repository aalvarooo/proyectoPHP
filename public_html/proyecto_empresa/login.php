<?php
require_once(__DIR__."/db_prueab.php");
if($_SERVER["REQUEST_METHOD"] == "POST") {

    $mysqli = $mysqli->prepare("SELECT `id` FROM `usuarios1` WHERE `correo` = ? OR `telefono` = ?");
    $sentencia->bind_param("ss", $_POST["email"], $_POST["telefono"]);
    $sentencia->execute();
